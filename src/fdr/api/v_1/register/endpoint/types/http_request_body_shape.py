...
class _Factory:
    def json(self, value: JsonRequestBody) -> HttpRequestBodyShape:
        if IS_PYDANTIC_V2:
            return HttpRequestBodyShape(root=_HttpRequestBodyShape.Json(**value.dict(exclude_unset=True), type="json"))  # type: ignore
        else:
            return HttpRequestBodyShape(
                __root__=_HttpRequestBodyShape.Json(**value.dict(exclude_unset=True), type="json")
            )  # type: ignore
...
class HttpRequestBodyShape(UniversalRootModel):
    ...
    def visit(
        self,
        json: typing.Callable[[JsonRequestBody], T_Result],
        ...
        if unioned_value.type == "json":
            return json(JsonRequestBody(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
